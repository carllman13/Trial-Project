...
(() => {
  ...
  function orderedFolders(folders) {
    let saved = [];
    try { saved = JSON.parse(localStorage.getItem(folderOrderKey) || '[]'); } catch { /* Browser storage may be disabled. */ }
  ...
  function saveFolderOrder() {
    try { localStorage.setItem(folderOrderKey, JSON.stringify(data.folders)); }
    catch { notice('Folder order changed for this session, but the browser could not save it.'); }
  }
  function moveFolder(source, target, after = false) {
    if (!source || !target || source === target) return;
    const folders = data.folders.filter(folder => folder !== source);
    const targetIndex = folders.indexOf(target);
    if (targetIndex < 0) return;
    folders.splice(targetIndex + (after ? 1 : 0), 0, source);
  ...
  document.addEventListener('click', async event => {
    ...
    switch (el.dataset.action) {
      ...
      case 'refresh-folders': { const result = await callBackend('refreshFolders'); if (result) { data.folders = orderedFolders(result); render(); } break; }
  ...
  app.addEventListener('reset', async () => { filterValues = { folder: '', fromDate: '', toDate: '', days: '', from: '', to: '', cc: '', subject: '', body: '', limit: '500', includeCC: false, evenings: false }; if (adapter) { const result = await callBackend('queryMessages', filterValues); if (result) { result.forEach(row => merge('messages', row)); state.results = result; } } else state.results = [...data.messages]; setTimeout(render, 0); });
  app.addEventListener('keydown', event => {
    if (event.altKey && event.target.matches('[data-folder]') && ['ArrowUp', 'ArrowDown'].includes(event.key)) {
      ...
      const index = data.folders.indexOf(source);
      const target = data.folders[index + (event.key === 'ArrowUp' ? -1 : 1)];
  ...
  async function start() {
    if (adapter) { app.textContent = 'Loading…'; try { if (!adapter.getInitialData) throw new Error('getInitialData is required'); data = await adapter.getInitialData(); data.folders = orderedFolders(data.folders); state.results = data.messages; state.folder = data.folders[0] || ''; state.checked = new Set(data.folders); state.chain = data.chains[0]?.id; state.message = data.messages[0]?.id; } catch (error) { app.textContent = `Unable to load backend data: ${error.message}`; return; } }
    else data.folders = orderedFolders(data.folders);
    render();
